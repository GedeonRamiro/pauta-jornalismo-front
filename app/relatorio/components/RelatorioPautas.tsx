...
const RelatorioPautas = ({ token }: Props) => {
  ...
  data.forEach((pauta) => {
    const name = pauta.user?.name || "Usuário desconhecido";
  ...
  data.forEach((pauta) => {
    const model = pauta.vehicle?.model || "Sem modelo";
    const manufacturer = pauta.vehicle?.manufacturer || "Sem fabricante";
  ...
  data.forEach((pauta) => {
    const name = pauta.camera?.name || "Câmera desconhecida";
  ...
  const resumoPautas = [
    ...
    { name: "Sem pautas", value: 0 },
  ];

  // ------------------------
  // 5. Pautas ao longo do tempo
  // ------------------------
